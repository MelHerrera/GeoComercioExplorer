...
def Get_actividades(request,codigo_postal, radio, actividad_to_search):
    ...
    m3 = folium.Map(location=[lat1,lon1], zoom_start=13)

    folium.Choropleth(
    geo_data=comercios_SHP.to_json(drop_id=True),
    name="Comercios",
    data=comercios_SHP,
    ...
    folium.Marker([lat1, lon1],  popup= "C.P. " + codigo_postal, icon=folium.Icon(color="green", icon="info-sign")).add_to(m3)
